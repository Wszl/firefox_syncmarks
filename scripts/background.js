...
function exportPHPMarks() {
	...
	chrome.bookmarks.getTree(function(bookmarkItems) {
		...
		chrome.storage.local.get(null, function(options) {
			...
			let cdata = "client="+s_uuid+"&ctype=chrome&caction=import&bookmark="+bookmarks;
...
function delMark(id, bookmark) {
	...
	chrome.storage.local.get(null, function(options) {
		...
		let cdata = "client="+s_uuid+"&ctype=chrome&caction=delmark&bookmark="+jsonMark;
		var xhr = new XMLHttpRequest();
		xhr.open("POST", options['wdurl'], true);
		xhr.setRequestHeader("Authorization", 'Basic ' + btoa(options['user'] + ":" + options['password']));
		xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
		...
		xhr.onload = function () {
			if( xhr.status < 200 || xhr.status > 226) {
				...
				if(response == 1)
						...
						console.log("Error: Bookmark not removed at the server, please check the server logfile");
			}
		}
		console.log("Info: Sending remove request to server. URL: "+bookmark.node.url+", Client: "+s_uuid);
		xhr.send(cdata);
	});	
	...
	chrome.browserAction.setTitle({title: chrome.i18n.getMessage("errorRemoveBookmark") + ": " + date.toLocaleDateString(undefined,doptions)});
}
	...
	chrome.storage.local.get(null, function(options) {
		chrome.bookmarks.get(bookmark.parentId, function(folder) {
			chrome.bookmarks.get(id, function(bmark) {
				let jsonMark = encodeURIComponent(JSON.stringify({ "id": id, "index": bookmark.index, "url": bmark[0].url, "folderIndex": folder[0]['index'],"folder": bookmark.parentId,"nfolder": folder[0]['title'] }));
				...
				let cdata = "client="+s_uuid+"&ctype=chrome&caction=movemark&bookmark="+jsonMark;
				...
				xhr.onload = function () {
					if( xhr.status < 200 || xhr.status > 226) {
						...
						if(response == 1)
								...
								console.log("Error: Bookmark not moved at the server, please check the server logfile");
					}
				}
				console.log("Info: Sending move request to server. Bookmark ID: "+id+", Client: "+s_uuid);
				xhr.send(cdata);
			});
		});
	});
...
function sendMark(bookmark) {
	...
	chrome.bookmarks.get(bookmark.parentId, function(bmark) {
		...
		chrome.storage.local.get(null, function(options) {
			...
			let cdata = "client="+s_uuid+"&ctype=chrome&caction=addmark&bookmark="+jsonMark;
			...
			xhr.onload = function () {
				if( xhr.status < 200 || xhr.status > 226) {
					...
					if(response == 1) {
						...
						loglines = logit("Error: Bookmark not added at the server, please check the server logfile: " + response);
					}
				}
			}
...
function getPHPMarks() {
	chrome.storage.local.get(null, function(options) {
		...
		let xhr = new XMLHttpRequest();
		let params = 'client='+s_uuid+'&ctype=chrome&caction=startup';
		...
		loglines = logit("Info: Sending parameters to server (client,type,action): "+s_uuid+", chrome, startup");
		xhr.send(params);
	});
}
...
function getAllPHPMarks() {
	chrome.storage.local.get(null, function(options) {
		...
		let xhr = new XMLHttpRequest();
		let params = 'client='+s_uuid+'&ctype=chrome&caction=export';
		xhr.open('POST', options['wdurl'] + '?t=' + Math.random(), true);
		xhr.setRequestHeader("Authorization", 'Basic ' + btoa(options['user'] + ":" + options['password']));
		xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
		xhr.withCredentials = true;
...
function importMarks(parsedMarks, index=0) {
	...
	chrome.bookmarks.create(
		...
		message = chrome.i18n.getMessage("errorImportBookmark") + '\"' + bmtitle + ' (' + bmurl + ')\"';
		notify('error', message);
		loglines = logit('Error: ' + message);
	}*/);
}
